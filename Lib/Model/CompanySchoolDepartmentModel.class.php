 ...
 *  数据库模型.
 */
class CompanySchoolDepartmentModel extends ArModel
{
    // 集团学校部门表
    public $tableName = 'u_company_school_department';
    ...
    {
        $sortData = array();
        $menu = $this->getAllMenuByDid(0, true, $condition);
        $titlePadMap = array(
            '│', '├─', '└─',
        );
        if ($menu) :
    ...
    // 获取所有分类 默认获取所有分类
    public function getAllMenuByDid($pid = 0, $getall = false, $condition = array())
    {
        $con['pdid'] = $pid;
        ...
        if (!$getall) :
            ...
            if (!empty($submenu)) :
                foreach ($submenu as &$sub_menu) :
                    $sub_menu['sub_menu'] = $this->getAllMenuByDid($sub_menu['did'], $getall);
                endforeach;
            endif;
        endif;
        return $submenu;
