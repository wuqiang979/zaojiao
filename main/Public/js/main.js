...
$(function () {
    ...
    //     }
    // });
    $('#SysLockScreen').click(function(){
        $('#lockScreenPage').slideDown(600,function(){
            $('#unlock input#password').focus()
        });
    })
    $('#lockScreenPage .unlockBtn').click(function(){
        var passValue = $(this).prev().find('input').val(),
            that = $(this);
        if (passValue == '') {
            that.prev().find('label').addClass('is-visible')
        }else {
            //Ajax请求~~
            //验证正确
            if (true) {
                $('#lockScreenPage').slideUp(600,function(){
    ...
    $('#unlock input#password').keydown(function(event){
        if (event.which == 13) {
            $('#unlock input#password').blur()
            $('#lockScreenPage .unlockBtn').trigger('click')
    ...
    $("#btnUserCenter").click(function (e) {
        $(".pagetitle").html("个人中心");
        $('#content').attr('src', JSV.PATH_SERVER+'Mine/info');
        $("#content").attr("url", JSV.PATH_SERVER+'Mine/info');
    ...
    $(".firstMenu,#logo").bind({
        ...
        click: function (e) {
            ...
            if (!isFloatMenuStatus || $(this).attr("id") == "logo") {
                ...
                $(".firstMenuSelectedTips").remove();
                // $this.addClass("selected").addClass("currentSelected").append("<span class='firstMenuSelectedTips'><img src='Images/ArrowLeft.png' /></span>");
            }
